...
public class LineSubmissionServiceTest {
    ...
    public void applyField_coercesBooleanFields() {
        LineSubmission s = new LineSubmission();
        LineSubmissionService.applyField(s, "backgroundChangeable", "yes");
        assertEquals(Boolean.TRUE, s.getBackgroundChangeable());

        LineSubmissionService.applyField(s, "singleAllelic", "no");
        assertEquals(Boolean.FALSE, s.getSingleAllelic());

        // Empty/garbage clears boolean fields back to null.
        LineSubmissionService.applyField(s, "backgroundChangeable", null);
        assertNull(s.getBackgroundChangeable());
    }
