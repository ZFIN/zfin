...
create temp table tmp_mrkr_figure(tmp_figure_zdb_id varchar(50), tmp_mrkr_zdb_id varchar(50)) with no log;

load from constructFigure.unl insert into tmp_mrkr_figure;
...
create temp table tmp_figure(tmp_fig_mrkr varchar(50),tmp_figure_zdb_id varchar(50), tmp_figure_pub_zdb_id varchar(50)) with no log;
...
unload to tmpfigmkr select m.tmp_mrkr_zdb_id,t.tmp_figure_zdb_id from tmp_mrkr_figure m, tmp_figure t where trim(m.tmp_figure_zdb_id)=trim(t.tmp_fig_mrkr);
insert into construct_figure (consfig_construct_zdb_id, consfig_fig_zdb_id) select m.tmp_mrkr_zdb_id,t.tmp_figure_zdb_id from tmp_mrkr_figure m, tmp_figure t where trim(m.tmp_figure_zdb_id)=trim(t.tmp_fig_mrkr);

!echo load image tables
create table tmp_image(tmp_img_zdb_id varchar(50),tmp_img_width integer, tmp_img_height integer, tmp_fig_zdb_id varchar(50),tmp_img_owner_zdb_id varchar(50),tmp_image_img varchar(150),tmp_img_thumbnail varchar(150));

load from images.unl insert into tmp_image;
...
unload to tmpimage select * from tmp_image;
unload to img_oldname_2_newname.txt select REPLACE(tmp_image_img,'.jpg',''),tmp_img_zdb_id from tmp_image;
unload to tmpfig select * from tmp_figure;
--rollback work;

create temp table tmp_dblink (dblink_id varchar(50), cnstrctid varchar(50),accnum varchar(50),fdbcontid varchar(50));
...
unload to 'newdblink' select * from tmp_dblink;
