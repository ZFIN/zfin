...
$out_file =  "sequences.gp"; 	#getparms("Output file", "refout", "");
...
$tool="esearch";
$results=getUrl("$ebase$tool.fcgi?usehistory=y&db=$db&term=$term&dopt=GenPept");
$results=~/<Count>(\d+)<\/Count>.*<QueryKey>(\d+).*<WebEnv>(.+)<\/WebEnv/s;
($nrecords,$qkey,$webenv)=($1,$2,$3);

#download in batches of 500 in user-specified format and write to output file
$tool="efetch";
$ts=time;

for($retstart=0;$retstart<$nrecords;$retstart+=$retmax){

    $t=time-$ts;if(3-$t>0){sleep(3-$t);}#max one call per 3 seconds for scripts at NCBI
