...
unload to 'alreadyinCND' select *  from candidate where cnd_suggested_name like 'ENSDARG%' and cnd_zdb_id not like 'ZDB-CND-131217%' and cnd_last_done_date is null and cnd_suggested_name in (select ensm_ensdarg_id from tmp_update);
...
unload to 'alreadyinCNDnotnull' select * from candidate  where cnd_suggested_name in (select ensm_ensdarg_id from tmp_update) and cnd_last_done_date is not null;
