#!/private/bin/perl -w
#-----------------------------------------------------------------------
# Subroutines used to pull Feature (ALT) information from EZRC.
#
# This file is 'required' by the pullFromResourceCenter.pl script.  This file is not
# meant to be run directly, but only to support the pullFromResourceCenter.pl script.
#
...
#----------------------------------------------------------------------
# Parse the downloaded file from EZRC.
#
...
#  $       DBI Database Handle
#  $       ZDB **LAB** ID of EZRC.
#  $       name of file downloaded from EZRC
...
sub geno_parse($$$) {
    ...
    open (EZRCGENOFILE,"$ezrcFile") || errorExit ("Failed to open $ezrcFile");

    my $cur = $dbh->prepare("
          insert into tmp_geno_pulled_from_ezrc
              (epfz_alt_zdb_id,
    ...
    while ($newGenoRequestLine = <EZRCGENOFILE>) {
...
sub geno_available_parse($$){
    ...
    my $genoId;
    my $dbh = $_[0];
    my $ezrcResourceFile = $_[1];

   open (EZRCRESOURCEFILE,"$ezrcResourceFile") || errorExit ("Failed to open $ezrcResourceFile");
    ...
    while ($newGenoAvailableLine = <EZRCRESOURCEFILE>) {
...
sub geno_load($) {
    ...
    my $insert_temp_1 = $dbh->prepare("insert into geno_pulled_from_ezrc (
                                                   epfz_alt_zdb_id,
       	    			                   epfz_background_zdb_id,
       	    			                   epfz_zygocity)
                                         select distinct epfz_alt_zdb_id,
			                                 epfz_background_zdb_id,
       	 		                                 epfz_zygocity
                                           from tmp_geno_pulled_from_ezrc;");

    my $insert_temp_clean = $dbh->prepare("update geno_pulled_from_ezrc
	...
	"insert into int_data_supplier (idsup_data_zdb_id, 
                                        ...
                                        idsup_acc_num)
           select epfz_geno_zdb_id, 'ZDB-LAB-130607-1', epfz_geno_zdb_id
             from geno_pulled_From_ezrc
             where not exists (select 'x' 
                                 from int_data_supplier
		                 where idsup_data_zdb_id = epfz_geno_zdb_id
		                 and idsup_supplier_zdb_id = 'ZDB-LAB-130607-1'
	...
	"insert into int_data_supplier (idsup_data_zdb_id, 
                                        idsup_supplier_zdb_id, 
                                        idsup_acc_num)
           select geno_zdb_id, 'ZDB-LAB-130607-1', geno_zdb_id
    ...
    my $delete_existing_where_supplied = $dbh->prepare("delete from geno_pulled_from_ezrc
                                                          where epfz_geno_zdb_id is not null;");

    my $update_genofeat=$dbh->prepare("update geno_pulled_from_ezrc
    ...
    my $add_new_genoids = $dbh->prepare("update geno_pulled_from_ezrc
	...
	"insert into int_data_supplier (idsup_data_zdb_id, idsup_acc_num, 
       	    		      			  idsup_supplier_zdb_id)
           select epfz_geno_zdb_id, epfz_geno_zdb_id, 'ZDB-LAB-130607-1'
             from geno_pulled_from_ezrc
             where not exists (Select 'x'
  	    	   	         from int_datA_supplier
			         where idsup_data_zdb_id = epfz_geno_zdb_id
    ...
    "insert into int_data_supplier (idsup_data_zdb_id, idsup_acc_num, 
       	    		      			  idsup_supplier_zdb_id)
               select distinct epfz_alt_zdb_id, epfz_alt_zdb_id, 'ZDB-LAB-130607-1'
                 from geno_pulled_from_ezrc
                 where not exists (Select 'x'
  	    	   	            from int_datA_supplier
    ...
    my $cur = $dbh->prepare("
          select idsup_data_zdb_id
            ...
            where get_obj_type(idsup_data_zdb_id) = 'GENO'
              and idsup_supplier_zdb_id = '$ezrcZdbId'
              and idsup_acc_num = idsup_data_zdb_id
              and not exists (select 'x'
                        from geno_available
                        where idsup_data_zdb_id = geno_zdb_id
                        and geno_zdb_id like 'ZDB-GENO%')
              and not exists (select 'x' from geno_pulled_from_ezrc
   ...
   ;");
    ...
    my $deleteCur = $dbh->prepare("
           delete from int_data_supplier
             where idsup_supplier_zdb_id = '$ezrcZdbId'
...
sub geno_altSuppliedByZFIN_count($$) {
    ...
    $cur->fetch();
    &writeReport(int($rowCount) . " Features from EZRC already being 'supplied' at ZFIN.\n");

    if ($rowCount == 0) {
	&errorExit("Aborting run!!!!  Something is probably wrong because the",
		   "  status of EVERY EZRC supplied ALT is changing.");
    }
    return ();
}
...
sub geno_GenoSuppliedByZFIN_count($$) {
    ...
    $cur->bind_columns(\$rowCount);
    $cur->fetch();
    &writeReport(int($rowCount) . " Genotypes from EZRC already being 'supplied' at ZFIN.\n");

    if ($rowCount == 0) {
	&errorExit("Aborting run!!!!  Something is probably wrong because the",
		   "  status of EVERY EZRC supplied GENO is changing.");
    }
    return ();
}
...
#----------------------------------------------------------------------
# Report any GENOs/ALTs in the list from EZRC that are not valid ZDB IDs
#
# Params
#  $       DBI Database Handle
#
...
sub geno_reportUnknownGenos($) {
    ...
    # report them
    my $cur = $dbh->prepare("select count(*) from geno_pulled_from_ezrc
                              where epfz_geno_zdb_id is not null;");
...
sub geno_reportSuppliedByEzrcGenos($) {
    ...
    $cur->bind_columns(\$rowCount);
    $cur->fetch();
    &writeReport(int($rowCount) ." GENOs currently supplied by EZRC.\n");

    return ();
}

sub geno_reportSuppliedByEzrcAlts($) {
    ...
    $cur->bind_columns(\$rowCount);
    $cur->fetch();
    &writeReport(int($rowCount) . " Features currently supplied by EZRC.\n");
...
# geno_main
#
# download GENO data from EZRC and update ZFIN to reflect what it says.
#
# Params
#  $       DBI Database Handle
#  $       ZDB ID of EZRC.
...
sub geno_main($$) {
    ...
    my $dbh = $_[0];
    my $ezrcZdbId = $_[1];
    my $ezrcGenoFile = "need.txt"; # geno file to download
    my $ezrcResourceFile = "resource.txt"; # which genos supplied from EZRC

    system("rm -f $ezrcGenoFile");    # remove old downloaded files
    system ("rm -f $ezrcResourceFile"); 

    &downloadFiles($ezrcGenoFile,"Genos");    # get new Geno file
    &downloadFiles ($ezrcResourceFile,"Genos"); # get Resource file

    &writeReport("\n*** need.txt and resource.txt files downloaded from EZRC. ***\n");

    # create temp table to load list of GENOs from EZRC into

    my $curFirstTable = $dbh->do('create temp table tmp_geno_pulled_from_ezrc
    ...
    my $curSecondTable = $dbh->do('create temp table geno_pulled_from_ezrc
          ( epfz_alt_zdb_id  varchar(50),
            epfz_background_zdb_id varchar(50),
            epfz_zygocity  varchar(40),
            epfz_geno_zdb_id varchar(50),
            epfz_ezrc_is_submitter boolean,
    ...
    &geno_parse($dbh,$ezrcZdbId,$ezrcGenoFile);
    &geno_available_parse($dbh,$ezrcResourceFile);

    &writeReport("*** Files parsed and loaded to temp tables. ***\n"); 

    &geno_reportSuppliedByEzrcAlts($dbh);
    &geno_reportSuppliedByEzrcGenos($dbh);
    &geno_altSuppliedByZFIN_count($dbh,$ezrcZdbId);
    &geno_GenoSuppliedByZFIN_count($dbh,$ezrcZdbId);
 
    &geno_load($dbh);

    &writeReport("*** Data Loaded. ***\n");

    &geno_dropDiscontinuedGenos($dbh,$ezrcZdbId);
    &geno_dropDiscontinuedAlts($dbh,$ezrcZdbId);
