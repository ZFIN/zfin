...
and not exists (select 1 from record_attribution where recattrib_data_zdb_id = dblink_zdb_id);

--!echo '		into temp table uniprot_records_without_attri'

create index urwoa_record_id_index
 on uniprot_Records_without_attri (uniprot_record_zdb_id);
...
and not exists (select 1 from record_attribution where recattrib_data_zdb_id = mrkrgoev_zdb_id);
...
and not exists (select 1 from record_attribution where recattrib_data_zdb_id = extnote_zdb_id);
