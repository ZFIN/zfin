...
system("/bin/rm -f sec_unload_report") and die "can not rm sec_unload_report" ;
...
&isEmptyFile ("term_no_longer_secondary.txt","No secondary terms are now primary","<!--|REQUEST_NEW_ANATOMY_EMAIL|-->","Error in quality.obo?  Terms once secondary are now primary.");
