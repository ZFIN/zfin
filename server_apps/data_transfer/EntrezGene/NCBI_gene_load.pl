...
system("/usr/bin/cp RefSeq-release60.catalog.gz RefSeqCatalog.gz"); 

system("/local/bin/gunzip RefSeqCatalog.gz");

system("/local/bin/wget ftp://ftp.ncbi.nih.gov/gene/DATA/gene2accession.gz");

system("/local/bin/wget ftp://ftp.ncbi.nih.gov/gene/DATA/gene2unigene");

system("/local/bin/wget -O zf_gene_info.gz ftp://ftp.ncbi.nih.gov/gene/DATA/GENE_INFO/Non-mammalian_vertebrates/Danio_rerio.gene_info.gz");

system("/local/bin/gunzip gene2accession.gz");

system("/local/bin/gunzip zf_gene_info.gz");
...
#--------------------------------------------------------------------------------------------------------------------
# execute the SQL file to prepare for 1) a delete list, toDelete.unl 2) a list of ZFIN genes to be mapped, toMap.unl
#--------------------------------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------
# Parse zf_gene_info file to get the NCBI records with gene Id, symbol, and Vega Id
#--------------------------------------------------------------------------------------------

open (ZFGENEINFO, "zf_gene_info") ||  die "Cannot open zf_gene_info : $!\n";

%vegaIdsNCBIids = %NCBIidsVegaIds = %geneSymbolsNCBIids = %NCBIidsGeneSymbols = %vegaIdwithMultipleNCBIids = ();
...
while (<ZFGENEINFO>) {
  ...
  if ($_) {
    ...
    if ($dbXrefs =~ m/Vega:(OTTDARG[0-9]+)/) {  ### if VEGA Id is there      
      ...
      if (exists($vegaIdwithMultipleNCBIids{$VegaIdNCBI}) || (exists($vegaIdsNCBIids{$VegaIdNCBI}) && $vegaIdsNCBIids{$VegaIdNCBI} ne $NCBIgeneId)) {        
      ...
      $NCBIidsVegaIds{$NCBIgeneId} = $VegaIdNCBI; 
    }
  }
...
print STATS "On NCBI's Danio_rerio.gene_info file, the following Vega Ids corresond to more than 1 NCBI genes\n\n";
print LOG "On NCBI's Danio_rerio.gene_info file, the following Vega Ids corresond to more than 1 NCBI genes\n";
...
#-------------------------------------------------------------------------------------------
## get ZFIN gene symbol / zdb gene id pairs

%geneSymbolsZDBids = %geneZDBidsSymbols = ();

$sqlGeneZDBidsSymbols = 'select mrkr_zdb_id, mrkr_abbrev from marker where mrkr_zdb_id like "ZDB-GENE%" and mrkr_abbrev not like "WITHDRAWN%";';
...
while ($curGeneZDBidsSymbols->fetch()) {
   ...
   $geneSymbolsZDBids{$symbol} = $zdbId;
}
...
%sequenceLength = ();  # key: seqence accession    # value: length
...
# key: sequence accession
# value: NCBI gene Id
%noLength = ();  # key: accession number

$ctNoLength = $ctlines = $ctZebrafishGene2accession = 0;
...
foreach $geneAtNCBI (sort keys %supportedGeneNCBI) {
   ...
   $ref_arrayAccs = $supportedGeneNCBI{$geneAtNCBI};
...
system("<!--|SOURCEROOT|-->/commons/bin/efetch.r -t fasta -i noLength.unl -o seq.fasta");
...
system("<!--|SOURCEROOT|-->/commons/bin/fasta_len.awk seq.fasta >length.unl");
...
open (ONETOZERO, ">oneToZero") ||  die "Cannot open ONETOZERO : $!\n";

$ctOneToZero = 0;
foreach $geneAtZFIN (sort keys %genesZFINwithNoRNAFoundAtNCBI) {
   $ref_arrayAccsNotFoundAtNCBI = $genesZFINwithNoRNAFoundAtNCBI{$geneAtZFIN};
...
foreach $geneAtNCBI (sort keys %genesNCBIwithAllAccsNotFoundAtZFIN) {
   $ref_arrayAccsNotFoundAtZFIN = $genesNCBIwithAllAccsNotFoundAtZFIN{$geneAtNCBI};
...
ZFINPerlModules->sendMailWithAttachedReport("<!--|GO_EMAIL_CURATOR|-->","$subject","reportNtoN");
...
ZFINPerlModules->sendMailWithAttachedReport("<!--|GO_EMAIL_CURATOR|-->","$subject","reportOneToN");
...
ZFINPerlModules->sendMailWithAttachedReport("<!--|GO_EMAIL_CURATOR|-->","$subject","reportNtoOne");
...
print LOG "\nThe following Vega Gene Ids at ZFIN corresponding to multiple ZDB Gene Ids\n";
...
foreach $zdbId (sort keys %geneZDBidsSymbols) {
  ...
  if (!exists($mapped{$zdbId}) && !exists($oneToNZFINtoNCBI{$zdbId}) && !exists($zdbGeneIdsNtoOneAndNtoN{$zdbId}) 
    ...
    if (exists($ZDBgeneAndVegaGeneIds{$zdbId}) && !exists($ZDBgeneWithMultipleVegaGeneIds{$zdbId})) {
      ...
      if (exists($vegaIdsNCBIids{$vegaGeneId}) && !exists($mappedReversed{$vegaGeneId}) 
           && !exists($vegaIdwithMultipleNCBIids{$vegaGeneId}) 
           && !exists($vegaGeneIdWithMultipleZFINgenes{$vegaGeneId}) 
           && !exists($geneNCBIwithAccSupportingMoreThan1{$vegaGeneId})) {
           
          ## ----- write the NCBI gene Ids mapped via Vega gene Id on toLoad.unl ---------------
          print TOLOAD "$zdbId|$vegaGeneId|||$fdcontNCBIgeneId|$pubMappedbasedOnVega|\n";
          
          $oneToOneViaVega{$vegaGeneId} = $zdbId;
          $ctMappedViaVega++;
      }
    }
  }
...
#---------------------------------------------------------------------------------------------
# prepare the final add-list for RefSeq and GenBank records 
#---------------------------------------------------------------------------------------------
...
$sqlGetGenBankAndRefSeqAccs = 'select dblink_linked_recid, dblink_acc_num, dblink_fdbcont_zdb_id, dblink_zdb_id 
                                 from db_link 
                                where dblink_linked_recid like "ZDB-GENE%"
...
# key: concatenated string of $gene,$acc and $fdbcont 
...
while ($curGetGenBankAndRefSeqAccs->fetch()) {
  if (!exists($toDelete{$dblinkId})) {              # exclude those to be deleted first
    $geneAccFdbcont{$gene . $acc . $fdbcont} = 1;
    $ctGeneAccFdbcont++;
  }
}
...
# ----------------  write GenBank RNA accessions on toLoad.unl ------------------
...
# -------------  write GenPept accessions on toLoad.unl  -----------------------
...
# -------------  write GenBank DNA accessions on toLoad.unl -----------------------
...
# ------------- write RefSeq RNA accessions on toLoad.unl ---------------------------
...
# --------------- write RefPept accessions on toLoad.unl ------------------------------
...
# ----------------- write RefSeq DNA accessions on toLoad.unl ------------------------------
...
#----------------------  include UniGene Ids in the toLoad list  -----------------------------
