...
-- INPUT VARS:
--	sbm_lab_name
--	sbm_mrkr_owner_id :  as owner of new marker	
--	sbm_img_owner_id :  as owner of new image record	
--	sbm_mrkr_pub_id  : pub attribution for markers
--	sbm_xpat_pub_id  : pub attribution for xpat and figures
...
create function gxp_load_func (
		...
		sbm_mrkr_owner_id	  like person.zdb_id,	
		sbm_img_owner_id	  like person.zdb_id,
		sbm_mrkr_pub_id	  like publication.zdb_id,
		sbm_xpat_pub_id	  like publication.zdb_id,
...
insert into record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id)
	select t_mrkr_zdb_id, sbm_mrkr_pub_id 
	  from tmp_gxp_marker;

-- add attribution to existing ESTs
insert into record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id)
	select tu_mrkr_zdb_id, sbm_mrkr_pub_id 
	  from tmp_gxp_updated_marker;
...
insert into tmp_gxp_fake_gene
  	 select get_id('GENE'), t_mrkr_zdb_id,
		case when sbm_fgene_prefix = "sc:"  --est: cssl:####
                     then sbm_fgene_prefix||t_mrkr_name[6,13]
		 when sbm_fgene_prefix = "im:"  --est: image:####
                     then sbm_fgene_prefix||t_mrkr_name[7,13]
		 when sbm_fgene_prefix = "gb:"  
                     then "expressed sequence "||upper(t_mrkr_name)
...
insert into record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id)
	select t_mrel_zdb_id, sbm_mrkr_pub_id from tmp_gxp_marker_relationship;
...
foreach
	...
	insert into figure (fig_zdb_id, fig_source_zdb_id,
		...
		values (curFigZdbId, sbm_xpat_pub_id, curCaption, "Fig. " || labelCounter);
...
if (sbm_lab_name = "Thisse" AND sbm_release_type = "cb") then 
  insert into image (
    ...
    select imgt_zdb_id, t_fig_zdb_id, 
	   ...
	   sbm_img_owner_id, imgt_image_name
