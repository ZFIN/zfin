...
elif (sbm_fgene_prefix == "sc:") then
   let originatorNote = "a cDNA generated by a lab at the Sanger Institute, Cambridge"; 
end if 
...
insert into tmp_gxp_fake_gene
  	 select get_id('GENE'), t_mrkr_zdb_id,
		case when sbm_fgene_prefix = "sc:"  --est: cssl:####
		 ...
		 when sbm_fgene_prefix = "im:"  --est: cssl:####
...
if (sbm_fgene_prefix == "sb:" and sbm_release_type == "eu") then
  ...
  update tmp_gxp_fake_gene set t_fgene_comments = 
		"This gene is characterized solely by a cDNA or colection of cDNAs. When more is known about the gene, the current gene nomenclature based on the placeholder name will be replaced with more traditional zebrafish gene nomenclature. " || t_fgene_comments
                         where t_fgene_name like "sc:";
