...
create index tnj_title_index
 on tmp_ncbi_journals(title) ;

create index tnj_issnPrint_index
 on tmp_ncbi_journals(issnPrint);

create index tnj_nlmID_index
 on tmp_ncbi_journals(nlmID);
...
create index lnj_issnPrint_index
 on lower_ncbi_journals(issnPrint);

create index lnj_nlmID_index
 on lower_ncbi_journals(nlmID) ;
...
create temp table duplJournalsNCBI as select j1.issnPrint, j1.title, j1.nlmID 
...
create temp table sameTitleNCBIjournals as select t1.title, t1.issnPrint, t1.nlmID
...
create temp table sameMedAbbrNCBIjournals as select t1.medAbbr, t1.issnPrint, t1.nlmID
  from lower_ncbi_journals t1
...
create temp table sameIsoAbbrNCBIjournals as select t1.isoAbbr, t1.issnPrint, t1.nlmID
  from lower_ncbi_journals t1
...
create temp table wrongIssnPrint as select j1.jrnl_zdb_id as journalZdbID, j1.jrnl_name, j1.jrnl_print_issn, issnPrint, j1.jrnl_abbrev, medAbbr, nlmID
...
\copy (select * from wrongIssnPrint)  to '<!--|TARGETROOT|-->/server_apps/data_transfer/PUBMED/Journal/wrongIssnPrintByTitle.txt' ;

create temp table wrongIssnPrintByMedAbbr as select j1.jrnl_zdb_id as journalZdbID, j1.jrnl_abbrev, j1.jrnl_print_issn, issnPrint, j1.jrnl_name, nlmID
  from journal j1, tmp_ncbi_journals
 where issnPrint is not null
   and j1.jrnl_print_issn is not null
...
\copy (select * from wrongIssnPrintByIsoAbbr) to '<!--|TARGETROOT|-->/server_apps/data_transfer/PUBMED/Journal/wrongIssnPrintByIsoAbbr.txt' ;
...
create temp table titleAlreadyThere as select issnPrint as issnP
...
create temp table titlesToChange as  select jrnl_print_issn as issn, jrnl_zdb_id as zdbid, jrnl_name as oldname, t.title as newname
...
create temp table problemRecords as  select t1.issn as issnD
...
\copy (select * from titlesToChange) to '<!--|TARGETROOT|-->/server_apps/data_transfer/PUBMED/Journal/titlesUpdated.txt' ;

-- report possible duplicated journals (same issn print, different zdb id)

create temp table  duplJournalsByIssnPrint as select j1.jrnl_print_issn, j1.jrnl_zdb_id, j1.jrnl_name, j1.jrnl_online_issn, j1.jrnl_abbrev, j1.jrnl_nlmid
  from journal j1
...
\copy (select * from duplJournalsByIssnPrint)  to '<!--|TARGETROOT|-->/server_apps/data_transfer/PUBMED/Journal/duplicateJournals.txt' ;
