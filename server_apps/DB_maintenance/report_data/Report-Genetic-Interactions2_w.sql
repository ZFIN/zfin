...
select mrkrgoev_zdb_id,mrkr_zdb_id,pub,string_agg(grouped_item,E'|') as inf,ont,term_ont_id into temp tmp_go1 from tmp_go
group by mrkrgoev_zdb_id,pub,ont,term_ont_id,mrkr_zdb_id having count(grouped_item) = 2 ;

select distinct mrkr_zdb_id,pub,inf,ont,term_ont_id into temp finalgo from tmp_go1 group by pub,inf,ont,term_ont_id,mrkr_zdb_id;

select distinct mrkr_zdb_id, concat(pub,'&',inf,'&',ont,'&',term_ont_id) as bundle, pub,inf,ont,term_ont_id into temp finalct3 from finalgo;




select pub,inf as messup,ont,term_ont_id as goid into temp jenk491 from finalct3 group by bundle,pub,inf,ont,term_ont_id having count(bundle)>1;



update jenk491 set messup=replace(messup,'|',',');
SELECT pub,unnest(string_to_array(messup, ',')) as inference, goid,ont,messup into temp jenk491b from jenk491;
...
select pub,  string_agg(affector,',') as affected_components,goid, ont,messup into temp tmp_final from final491 group by pub,goid,ont,messup;
