...
&sendResult("Monthly ZGC statistics","<!--|COUNT_ZGC_OUT|-->", "./zgcStatistics.txt");
&sendResult("Monthly ZGC statistics Err", "<!--|COUNT_ZGC_ERR|-->", "./err.txt");
...
&sendResult("Vega-Thisse statistics Err", "<!--|COUNT_THISSE_VEGA_ERR|-->", "./err.txt");


system("$ENV{'INFORMIXDIR'}/bin/dbaccess -a <!--|DB_NAME|--> VegaCount.sql > VegaStatistics.txt 2> err.txt");

&sendResult("Monthly Vega statistics", "<!--|COUNT_VEGA_OUT|-->","./VegaStatistics.txt");
&sendResult("Monthly Vega statistics Err", "<!--|COUNT_VEGA_ERR|-->", "./err.txt");
