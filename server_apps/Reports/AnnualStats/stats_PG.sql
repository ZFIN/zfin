...
insert into annual_stats(as_count, as_section, as_type, as_date)
select count(distinct xpatex_gene_zdb_id), 'Expression & Phenotype', 'Genes wiht expression data', now() from expression_experiment2, clean_expression_fast_search
...
select year(as_date), as_pk_id, as_section, as_type, as_count from annual_stats
 order by year(as_date) desc, as_pk_id asc) to 'stats.txt';
