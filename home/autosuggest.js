...
/******************************************************
Theo Larrieu's enhancement of autosuggest for each item
in a multiitem list.

Peiran's edit on word match for multiword target string,
and replace for (i in NodeList) into typical for loop syntax
to work around Safari bug.

Peiran's edit on adding this.div.onmouseout
...
function AutoSuggest(elem, suggestions)
{
	...
	********************************************************/
	...
	/********************************************************
	onmouseout handler for the this.div
	The div stick around in case no manual selection. This func 
	helps to release it. It would be great to add handler onto the 
	elem too, but the div onmouseover might not get a chance
        to be in effect when elem onmouseout hide the div.
	********************************************************/
	this.div.onmouseout = function(ev) 
	{
		me.hideDiv();
	}

	/********************************************************
	onmouseover handler for the this.div
	When mouse over, keep the list.
	********************************************************/
	...
	********************************************************/
	this.useSuggestion = function()
	{
		...
		{
			//debugStr += "-useSuggestion-" + this.preserveStr + " " + this.eligible[this.highlighted];
	...
	this.getEligible = function()
	{
		...
		for (var i=0 ; i < this.suggestions.length ; i++) {
			...
			// add the second condition to enable individual word match to a 
                        // multiwords target string
			if(suggestion.toLowerCase().indexOf(this.inputText.toLowerCase()) == "0" ||
			   suggestion.toLowerCase().indexOf(" "+this.inputText.toLowerCase()) > 0)
			{
	...
	********************************************************/
	this.getKeyCode = function(ev)
	{
		...
		var theKeyCode = (ev.keyCode) ? ev.keyCode : ((ev.which) ? ev.which : 0);
		return theKeyCode;
		***/
