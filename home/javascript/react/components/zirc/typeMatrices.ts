...
export const LESION_FIELD_DEFS: Record<LesionFieldKey, LesionFieldDef> = {
    ...
    insertionSizeBp:   {key: 'insertionSizeBp',   label: 'Insertion size',              type: 'int',     suffix: 'bp'},
...
const BASE_LESION_FIELDS_BY_TYPE: Record<LesionTypeKey, LesionFieldKey[]> = {
    ...
    // Indel (delins) has both a deletion size + sequence and an insertion
    // size + sequence. lesionSizeBp is reused as the deletion size; the
    // schema relabels it "Deletion size" for indel rows so the two sizes
    // are unambiguous on the form.
    indel:          ['lesionSizeBp', 'insertionSizeBp', 'deletedSequence', 'insertedSequence', 'locationInline', 'hasLargeVariant', 'mutatedAminoAcids', 'mutatedAminoAcidsHgvs', 'additionalInfo'],
