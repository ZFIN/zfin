        ...
        <tr>
            ...
            <authz:authorize ifAnyGranted="root">
                <c:if test="${!empty editURL}">
                    <td>
                        <a href="${editURL}" class="root">Add/Update this Record</a>
                    </td>
                </c:if>
                <c:if test="${!empty deleteURL
                  and
                  !fn:startsWith('ZDB-TSCRIPT-',oboID)
                  }">
                    <td>
                        <a href="${deleteURL}" class="root">Delete or Merge this record</a>
                    </td>
                </c:if>
                ...
                <td>
                <a href="/<%= ZfinProperties.getWebDriver()%>?MIval=aa-update-vframeset.apg&OID=${zdbID}&rtype=${rtype}">
