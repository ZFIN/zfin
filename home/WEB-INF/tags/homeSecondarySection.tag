...
<authz:authorize access="hasRole('root')">
    ...
    <script type="text/javascript">
        $('#pubID').bind('keypress', function (e) {
            // if the key pressed is the enter key
            if (e.which == 13) {
                var pubID = $('#pubID').val();
                if (pubID.indexOf('ZDB-PUB-') != '0') {
                    pubID = 'ZDB-PUB-' + pubID;
                }
                window.location.replace("/action/curation/" + pubID);
            }
        });
        </script>
...
<ul>
    ...
    <li>
        <a href="https://@WIKI_HOST@/display/general/Other+Databases">Other Fish Genomes and Model Organism
            Databases</a>
    </li>
