...
<zfin2:subsection title="DISEASE ASSOCIATED WITH <i>${gene.abbreviation}</i> HUMAN ORTHOLOG<a class='popup-link info-popup-link' href='/action/marker/note/omim-phenotype'></a>"
                  test="${!empty gene.diseaseDisplays}" showNoData="true">
    ...
    <table class="summary rowstripes marker-go-table">
        ...
        <c:forEach var="row" items="${gene.diseaseDisplays}" varStatus="loop">
            <zfin:alternating-tr loopName="loop" groupBeanCollection="${gene.diseaseDisplays}" groupByBean="diseaseTerm" newGroup="true">
                <td>
                    <zfin:groupByDisplay loopName="loop" groupBeanCollection="${gene.diseaseDisplays}" groupByBean="diseaseTerm">
