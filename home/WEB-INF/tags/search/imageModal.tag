...
<div id="${modalDomID}" class="modal hide fade">
    ...
    <div class="modal-body">
        ...
        <c:choose>
            <c:when test="${fn:length(result.images) == 1}">
                <img src="/imageLoadUp/${result.image}"/>
            </c:when>
            <c:when test="${fn:length(result.images) > 1}">
                <div id="${carouselDomID}" class="carousel slide">
                    ...
                    <div class="carousel-inner">
                        <c:forEach var="image" items="${result.images}" varStatus="loop">
                            <div class="<c:if test="${loop.index == 0}">active</c:if> item">
                               <img src="/images/ajax-loader.gif" realsrc="/imageLoadUp/${image}"/>
                            </div>
                        </c:forEach>
                    </div>
