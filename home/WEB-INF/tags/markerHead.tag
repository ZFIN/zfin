...
<table class="primary-entity-attributes">
    ...
    <c:if test="${formBean.marker.type ne 'EFG'&& formBean.marker.type ne 'EREGION'&& formBean.marker.type ne 'CNE' && !(fn:contains(formBean.marker.type,'CONSTRCT'))}">
    ...
    <c:if test="${formBean.marker.type eq 'CNE'}">
        <%--<c:if test="${formBean.marker.type ne 'REGION'}">--%>
        <tr>
            <th>Location:</th>
            <td>
                <zfin2:displayLocation entity="${formBean.marker}" hideLink="true"/>
            </td>
        </tr>
    </c:if>
