...
<%@attribute name="summary" type="org.zfin.figure.presentation.FigureExpressionSummary" rtexprvalue="true"
             required="true" %>
...
<c:if test="${!empty summary.startStage}">
    <table class="primary-entity-attributes">
            ...
            <tr>
                <th>
                    <zfin:choice choicePattern="0#Genes:| 1#Gene:| 2#Genes:"
                                 integerEntity="${fn:length(summary.genes)}"/>
                </th>
            ...
            <tr>
                <th>
                    <zfin:choice choicePattern="0#Antibodies:| 1#Antibody:| 2#Antibodies:"
                                 integerEntity="${fn:length(summary.antibodies)}"/>
                </th>
        ...
        <c:if test="${!empty summary.experiments}">
            <tr>
                <th>
                    <zfin:choice choicePattern="0#Conditions:| 1#Condition:| 2#Conditions:"
                                 integerEntity="${fn:length(summary.experiments)}"/>
                </th>
        ...
        <c:if test="${!empty summary.sequenceTargetingReagents}">
            <tr>
                <th>
                    <zfin:choice choicePattern="0#Knockdown Reagents:| 1#Knockdown Reagent:| 2#Knockdown Reagents:"
                                 integerEntity="${fn:length(summary.sequenceTargetingReagents)}"/>
                </th>
                <td><zfin2:toggledLinkList collection="${summary.sequenceTargetingReagents}" maxNumber="5"
                                           commaDelimited="true"/></td>
            </tr>
        </c:if>
        <c:if test="${!empty summary.entities}">
            <tr>
                <th>
                    ...
                    <zfin:choice choicePattern="0#Anatomical Terms:| 1#Anatomical Term:| 2#Anatomical Terms:"
                                 integerEntity="${fn:length(summary.entities)}"/>
                </th>
        ...
        <c:if test="${!suppressProbe}">
            <c:if test="${!empty summary.probe}">
                <tr>
                    ...
                    <td>
                        ...
                        <c:if test="${!empty summary.probe.rating}">
                            ...
                            <zfin2:starRating rating="${summary.probe.rating}"/>
                        </c:if>
                    </td>
                </tr>
                ...
                <c:if test="${!empty summary.probeSuppliers}">
                    <tr>
                        <th>
                            <zfin:choice choicePattern="0#Suppliers:| 1#Supplier:| 2#Suppliers:"
                                         integerEntity="${fn:length(summary.probeSuppliers)}"/>
                        </th>
