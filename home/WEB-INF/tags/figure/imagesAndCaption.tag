                    ...
                    <c:forEach var="image" items="${figure.images}">
                        <c:choose>
                            <c:when test="${image.videoStill}">
                                <video controls ${autoplay} loop height="500" poster="/imageLoadUp/${image.medium}">
                            ...
                            <c:otherwise>
                                <zfin:link entity="${image}">
                                    <c:choose>
                                        <c:when test="${!empty image.imageWithAnnotationMediumFilename}">
                                            <c:set var="filename" value="${image.imageWithAnnotationMediumFilename}"/>
                                        </c:when>
                                        <c:otherwise>
                                            <c:set var="filename" value="${image.medium}"/>
                                        </c:otherwise>
                                    </c:choose>
