    ...
    <c:forEach var="displayGroup" items="${formBean.displayGroups}">
        <div class="summary">
            <table class="summary largestripes">
                <tr class="odd oddgroup">
                    <td width="10%">Group:</td>
                    <td><b>${displayGroup.groupName}</b></td>
                </tr>
                <tr class="even evengroup newgroup">
                    <td>Definition:</td>
                    <td>
                        <small>${displayGroup.definition}</small>
                    </td>
                </tr>
                <tr class="odd evengroup">
                    <td>Members:</td>
                    <td>
                        <c:forEach var="refDB" items="${displayGroup.referenceDatabases}" varStatus="loop">
                            <small>${refDB.foreignDB.dbName} ${refDB.foreignDBDataType.dataType.toString()} <c:if
                                    test="${!loop.last}">,</c:if></small>
                        </c:forEach>
                    </td>
            </table>
        </div>
