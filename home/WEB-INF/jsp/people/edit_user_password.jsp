...
function cancel() {
    window.location.replace("/<%=ZfinProperties.getWebDriver()%>?MIval=aa-persview.apg&OID=${formBean.user.zdbID}");
}
...
<%--
  <form name=login_form METHOD=GET action="/action/webdriver/modify-password">
--%>
<c:choose>
                ...
                <TABLE>
                    <TR align=center>
                        ...
                        <TD><font size=+2>
                            ...
                            <br> (<font size=-1>${formBean.user.zdbID} </font>)
                        </TD>
                    </TR>
                </TABLE>
            </TD>
...
<authz:authorize ifAllGranted="root">
    <TABLE width=100% cellpadding=5>
        <TR align=center>
            <TD>
                <b>LOGIN:</b>
                <form:input path="user.login" size="12" onclick="check_login(1)" maxlength="12" id="login"/>
            ...
            <TD>
                ...
                <form:select path="user.role">
                    ...
                    <c:if test="${formBean.user.role eq 'root'}">
                        <form:option value="root"/>
                    </c:if>
                </form:select>
                <form:errors path="user.role" cssClass="Error"/>
            </TD>
        </TR>
    </TABLE>
</authz:authorize>
<authz:authorize ifAllGranted="submit">
    <TABLE width=100% cellpadding=5>
        <TR align=center>
            <TD>
                <b>LOGIN:</b>
                <form:hidden path="user.login" id="login"/>
                    ${formBean.user.login}
            </TD>
            <TD>
                <b>ACCESS:</B>
                <input type="hidden" name="user.role" value="${formBean.user.role}"/>
                    ${formBean.user.role}
            </TD>
        </TR>
    </TABLE>
...
<TABLE width=100% cellpadding=5>
    ...
    <TR align=center>
        <TD><b>PASSWORD:</b>
            <form:password path="passwordOne" onchange="check_passw(2);" size="8" maxlength="8" id="password"/>
            <form:errors path="passwordOne" cssClass="Error"/>
        </TD>
        <TD>
            <b>Re-Type PASSWORD</b> <font size=-1> (to verify)</font>:
            <form:password path="passwordTwo" onchange="check_passw2(4);" size="8" maxlength="8" id="password2"/>
