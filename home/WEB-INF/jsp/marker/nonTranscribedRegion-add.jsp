...
<div class="container-fluid">
    ...
    <form:form id="nonTranscribedRegion-add" class="form-horizontal" commandName="formBean"
        ...
        <div class="form-group">
            <form:label path="type" class="col-sm-2 control-label">Type</form:label><a
                href="http://www.sequenceontology.org/browser/current_svn/term/">Link to the SO (for
            reference)</a>
            <div class="col-sm-4">
        ...
        <div id="term-info" class="jq-modal curation"
             style="width: 600px; height: 500px; padding: 15px 15px;"></div>
        <div class="form-group">
...
<script>
    ...
    $(document).ready(function () {
        $('#type').change(function () {
            ...
            oboID = value.split('|')[0];
            openModalPopup('term-info');
            jQuery("#term-info").load('/action/ontology/term-detail-popup?termID=' + oboID)
        });
    });
    $('#term-info').on('show.bs.modal', function () {
        jQuery('.jq-modal ').css('overflow-y', 'auto');
        jQuery('.jq-modal ').css('max-height', $(window).height() * 0.7);
    });
