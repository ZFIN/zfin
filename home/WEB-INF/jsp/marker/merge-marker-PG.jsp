...
<authz:authorize access="hasRole('root')">
    ...
    <script type="text/javascript">
            ...
            if (eapPubs.length > 0) {
                for (eapPub in eapPubs) {
                    ...
                    if (numberOfEapPubs == 1)
                        jQuery('#validationEapText').append('<h3><a target="_blank" href="/${formBean.zdbIDToDelete}">${formBean.markerToDeleteViewString}</a> has been in the following publication(s) with expression as phenotype data:</h3>');


                    jQuery('#validationEapText').append('<div>'
                            + '<a target="_blank" href="/' + eapPubs[eapPub].publicationZdbId +'">'
                            + eapPubs[eapPub].linkContent + '</a>'
                            + '</div>');
                }
                jQuery('#ignoreEap').show();
            } else {
                eapIgnored = true;
            }
        ...
        function ignoreEap(formObj) {
            eapIgnored = true;
            jQuery('#validationEapText').hide();
            jQuery('#ignoreEap').hide();
        ...
        function enableMerge() {
            if (unspecifiedAllelesIgnored && sequenceTargetingReagentsIgnored && antibodiesIgnored && ncbiGeneIdsIgnored && uniGeneIdsIgnored && vegaIdsIgnored && EnsemblGRCz11IdsIgnored && transcriptsIgnored && orthologyIgnored && nonEapIgnored && eapIgnored && mapInfoIgnored && !differentSequence && !differentFish && !differentTargets) {
                jQuery('#submitMerge').removeAttr('disabled');
            }
        }
    ...
    <c:if test="${formBean.markerToDelete.markerType.name ne 'ATB'}">
        ...
        <form id="ignoreEap">
            <input type="button" value="Ignore Eap" onclick="ignoreEap(this);" title="By clicking this button, you acknowledge the fact that after the merge is done, the eap data of ${formBean.markerToDeleteViewString} will be associated into the gene retained.">
        </form>
