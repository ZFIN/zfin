...
<div class="summary">
    ...
    <c:if test="${!formBean.nonWildtypeSTRExist}">
        No data available
    </c:if>
    <c:if test="${formBean.nonWildtypeSTRExist}">
        <table class="summary rowstripes">
            ...
            <c:forEach var="sequenceTargetingReagentStat" items="${formBean.nonWildtypeSTRs}" varStatus="loop">
        ...
        <c:if test="${!formBean.allMutantSTRsDisplayed}">
            <table width="100%">
                <tbody>
                <tr align="left">
                    <td>
                        Show all
                        <a href="/action/ontology/show-all-sequence-targeting-reagents/${formBean.aoTerm.zdbID}/false">
                            ${formBean.mutantSTRcount}
                            <zfin:choice choicePattern="0# experiments| 1# experiment| 2# experiments"
                                         integerEntity="${formBean.mutantSTRcount}"/>
                        </a>
                    </td>
                </tr>
                </tbody>
