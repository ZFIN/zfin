            ...
            <td>${entityTrace.action}
                <c:if test="${entityTrace.action == 'MODIFY'}">
                    ...
                    <a onclick="jQuery('#showNameUpdate-${index.index}').hide();
                            ...
                            jQuery('#nameUpdate-${index.index}').load('/action/unload/fetch-changed-entity-records?entityID=${entityTrace.entityId}&tableName=${entityTrace.tableName}&date=${entityTrace.date}', function() { processPopupLinks(); }); return false;"
