    ...
    <small>
        <c:choose>
            <c:when test="${formBean.onlyFiguresWithImg}">
                [ <a
                    href="javascript:document.location.replace('figure-summary?antibody.zdbID=${formBean.antibody.zdbID}&superTerm.ID=${formBean.superTerm.zdbID}&subTerm.ID=${formBean.subTerm.zdbID}<c:if test="${formBean.startStage.zdbID != null}">&startStage.zdbID=${formBean.startStage.zdbID}&endStage.zdbID=${formBean.endStage.zdbID}</c:if>&onlyFiguresWithImg=false')">
                Show all figures</a> ]
            </c:when>
            <c:otherwise>
                [ <a
