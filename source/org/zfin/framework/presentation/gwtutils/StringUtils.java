...
public class StringUtils implements IsSerializable {
     ...
     * Needed for bizare behavior in GWT
     *
     * @param one string
     * @param two sgring
     * @return boolean
    ...
    public static boolean equalsWithNullStringOrEmpty(String one, String two) {
        if (one != null && (one.equals(NULL) || isEmpty(one)))
            one = null;
        if (two != null && (two.equals(NULL) || isEmpty(two)))
            two = null;
        return equals(one, two);
    }
}
