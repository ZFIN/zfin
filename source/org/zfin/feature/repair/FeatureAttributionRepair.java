...
public class FeatureAttributionRepair extends AbstractScriptWrapper {
    ...
    public void scanThroughPublicationsAndFix() {
...
//        List<Publication> publications = Collections.singletonList(getPublicationRepository().getPublication("ZDB-PUB-970327-24"));
    ...
    private void attributeGenesToPublication(Publication publication, Set<Marker> targetedGenes, String intermediaryAbbreviation, String intermediaryID, String reason) {
        for(Marker gene : targetedGenes) {
            if (getInfrastructureRepository().getRecordAttribution(gene.zdbID, publication.getZdbID(), RecordAttribution.SourceType.STANDARD) == null) {
                ...
                logger.info("SQL INSERT (updates) --> " +
                "INSERT INTO updates (submitter_id,rec_id,field_name,new_value,comments,submitter_name) " +
                ...
                logger.info("SQL INSERT (record_attribution) --> " +
                        "INSERT INTO record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id, recattrib_source_type) VALUES ('" + gene.getZdbID() + "', '" + publication.getZdbID() + "', 'standard') ON CONFLICT DO NOTHING;");
            }
        }
    }
