...
public class HibernateMarkerRepository implements MarkerRepository {
    ...
    public List<Marker> getGenesByAbbreviation(String name) {
        List<Marker> markerList = currentSession().createQuery("from Marker where upper(abbreviation) like :name || '%' " + " AND zdbID like 'ZDB-GENE%' order by abbreviationOrder asc ", Marker.class).setParameter("name", name.toUpperCase()).list();


        List<Marker> markerListContains = currentSession().createQuery("from Marker where upper(abbreviation) like  '%' || :name || '%' " + " AND zdbID like 'ZDB-GENE%' order by abbreviationOrder asc ", Marker.class).setParameter("name", name.toUpperCase()).list();
        markerListContains.forEach(familyName -> {
            if (!markerList.contains(familyName)) markerList.add(familyName);
        });
        return markerList;
    ...
    public List<Marker> getMarkersForRelation(String featureRelationshipName, String publicationZdbID) {
        String sql = "SELECT DISTINCT mrkr_zdb_id " + "    FROM marker,marker_type_group_member, " + "    record_attribution" + "    WHERE mrkr_zdb_id = recattrib_data_zdb_id" + "    AND recattrib_source_zdb_id=:pubZdbID " + "    AND mrkr_type=mtgrpmem_mrkr_type AND mtgrpmem_mrkr_type_group='CONSTRUCT_COMPONENTS'";
    ...
    public List<MarkerRelationship> getMarkerRelationshipsByPublication(String publicationZdbID) {
        ...
        Session session = currentSession();
        String hql = "select distinct mr from MarkerRelationship as mr, " + "PublicationAttribution as attribution " + "where  attribution.dataZdbID = mr.zdbID AND " + "mr.type in (:markerRelationshipType)AND " + "attribution.publication.zdbID = :pubID ";
    ...
    public List<String> getMarkerRelationshipTypesForMarkerEdit(Marker marker, Boolean interacts) {
        ...
        if (interacts) {
            ...
            String hql = "select mr.name from MarkerRelationshipType mr " + " where mr.firstMarkerTypeGroup.name in (:mTypeGroup) and mr.name not like '%interacts%'";
            Query<String> query = session.createQuery(hql, String.class);
            query.setParameterList("mTypeGroup", mTypeGroup);
            return query.list();
        }
    ...
    public MarkerRelationship addMarkerRelationship(MarkerRelationship mrel, String sourceZdbID) {
        ...
        String updateComment = "Creating relationship \"" + mrel.getFirstMarker().getAbbreviation() + " " + mrel.getMarkerRelationshipType().getFirstToSecondLabel() + " " + mrel.getSecondMarker().getAbbreviation() + "\" with Attribution: " + sourceZdbID;
    ...
    public boolean hasSmallSegmentRelationship(Marker associatedMarker, Marker smallSegment) {
        ...
        String hql = "from MarkerRelationship where firstMarker = :firstMarker AND secondMarker = :secondMarker " + " AND (type = :type1 or type = :type2 or type = :type3) ";
        Query<MarkerRelationship> query = session.createQuery(hql, MarkerRelationship.class);
    ...
    public List<Marker> getMarkersByAbbreviationAndGroup(String name, Marker.TypeGroup markerType) {
        ...
        String hql = " select distinct m from Marker m  " + " where m.abbreviation like :name  " + " and m.markerType in (:types)  ";
//                + " order by m.abbreviationOrder asc " ;
    ...
    public List<Marker> getConstructsByAttribution(String name) {
        String hql = " select distinct m from Marker m , PublicationAttribution pa " + " where lower(m.name) like lower(:name)  " + " and pa.dataZdbID = m.zdbID  " + " and m.markerType like '%CONS%'  ";
//                + " order by m.abbreviationOrder asc " ;
    ...
    public Marker getMarkerByAbbreviationAndAttribution(String name, String pubZdbId) {
        String hql = " select distinct m from Marker m , PublicationAttribution pa " + " where trim(m.abbreviation) = trim(:name)  " + " and pa.dataZdbID = m.zdbID  " + " and pa.sourceZdbID = :publicationZdbId ";
    ...
    public boolean getGeneHasExpressionImages(Marker gene) {
        String hql = " " + " select count( figs ) from ExpressionExperiment ee " + " join ee.expressionResults er " + " join er.figures figs " + " join figs.images ims " + " where ee.gene.zdbID = :geneZdbID ";
        Query<Number> query = currentSession().createQuery(hql, Number.class);
        query.setParameter("geneZdbID", gene.getZdbID());
        return (query.uniqueResult().longValue() > 0);
    }
    ...
    public boolean getGeneHasExpression(Marker gene) {
        String hql = " " + " select count( er) from ExpressionExperiment ee " + " join ee.expressionResults er " + " join er.figures figs " + " where ee.gene.zdbID = :geneZdbID ";
        Query<Number> query = currentSession().createQuery(hql, Number.class);
        query.setParameter("geneZdbID", gene.getZdbID());
        return (query.uniqueResult().longValue() > 0);
    }
    ...
    public boolean getGeneHasPhenotype(Marker gene) {
        String sql = "SELECT count(phenox_pk_id) " + "FROM mutant_fast_search, phenotype_experiment " + "WHERE mfs_data_zdb_id = :geneZdbID " + "AND mfs_genox_zdb_id = phenox_genox_zdb_id ";
    ...
    public boolean getGeneHasPhenotypeImage(Marker gene) {
        String sql = " " + "SELECT phenox_fig_zdb_id " + "FROM mutant_fast_search, phenotype_experiment, image " + "WHERE mfs_data_zdb_id = :geneZdbID " + "AND mfs_genox_zdb_id = phenox_genox_zdb_id " + "AND phenox_fig_zdb_id = img_fig_zdb_id";
        Query query = currentSession().createSQLQuery(sql);
        query.setParameter("geneZdbID", gene.getZdbID());
    ...
    public PaginationResult<HighQualityProbe> getHighQualityProbeStatistics(GenericTerm aoTerm, PaginationBean pagination, boolean includeSubstructures) {
        ...
        if (includeSubstructures) {
            hqlCount = "select count(distinct stat.probe) " + "     from HighQualityProbeAOStatistics stat " + "     where stat.superterm.zdbID = :aoterm ";
        } else {
            hqlCount = "select count(distinct stat.probe) " + "     from HighQualityProbeAOStatistics stat " + "     where stat.superterm.zdbID = :aoterm and " + "           stat.subterm.zdbID = :aoterm ";
        }
        ...
        String sqlQueryStr = " select distinct(stat.fstat_feat_zdb_id), probe.mrkr_abbrev as probeAbbrev, gene.mrkr_zdb_id," + "                       gene.mrkr_abbrev,gene.mrkr_abbrev_order  " + "from feature_stats as stat, marker as gene, marker as probe " + "     where fstat_superterm_zdb_id = :aoterm " + "           and fstat_gene_zdb_id = gene.mrkr_zdb_id " + "           and fstat_feat_zdb_id = probe.mrkr_zdb_id " + "           and fstat_type = :type ";
        if (!includeSubstructures) {
...
*/
    ...
    public List<Marker> getMarkersForStandardAttributionAndType(Publication publication, String type) {
        ...
        String hql = "select m from PublicationAttribution pa , Marker m " + " where pa.dataZdbID=m.zdbID and pa.publication.zdbID= :pubZdbID  " + " and pa.sourceType= :sourceType and m.markerType.name in (:types) " + " order by m.abbreviationOrder ";
        Query<Marker> query = HibernateUtil.currentSession().createQuery(hql, Marker.class);
    ...
    public List<Marker> getMarkersForAttribution(String publicationZdbID) {
        String hql = "" + " select distinct m from Marker m , RecordAttribution ra " + " left join fetch m.aliases " + " left join fetch m.markerType " + " where ra.dataZdbID=m.zdbID and ra.sourceType = :standard and ra.sourceZdbID = :pubZdbID " + " order by m.abbreviationOrder " + " ";
    ...
    public List<ConstructCuration> getConstructsForAttribution(String publicationZdbID) {
        String hql = "" + " select distinct m from ConstructCuration m , RecordAttribution ra " + " where ra.dataZdbID=m.zdbID and ra.sourceType = :standard and ra.sourceZdbID = :pubZdbID " + " ";
    ...
    public List<Publication> getHighQualityProbePublications(GenericTerm anatomyTerm) {
        Session session = HibernateUtil.currentSession();
        String hql;
        hql = "select distinct stat.publication" + "     from HighQualityProbeAOStatistics stat " + "     where stat.superterm = :aoterm and " + "           stat.subterm = :aoterm" + "     order by stat.publication.publicationDate ";
    ...
    public List<Marker> getTargetGenesAsMarkerForSequenceTargetingReagent(SequenceTargetingReagent stReagent) {
        ...
        Session session = currentSession();
        String hql = "select rel.secondMarker from MarkerRelationship as rel  " + "where rel.firstMarker = :sequenceTargetingReagent and rel.type = :type " + "order by rel.secondMarker.abbreviationOrder";
        Query<Marker> query = session.createQuery(hql, Marker.class);
    ...
    public List<String> getNMarkersWithUniProtNote(int firstNIds) {
        Session session = HibernateUtil.currentSession();
        String hql = "select distinct accession.dataZdbID from DBLink accession, ExternalNote note" + " where accession.referenceDatabase.foreignDB.dbName = :sourceName " + " and note.externalDataZdbID = accession.zdbID " + " order by accession.dataZdbID";
        Query<String> query = session.createQuery(hql, String.class);
    ...
    public List<MarkerRelationshipPresentation> getRelatedMarkerOrderDisplayExcludeTypes(Marker marker, boolean is1to2, MarkerRelationship.Type... typesNotIn) {
        String sql1To2 = " 	select mrkr_abbrev, mrkr_zdb_id, mrkr_abbrev_order, mrkrtype_type_display,  " + "	       mreltype_1_to_2_comments, " + "          '<a href=\"/'||mrkr_zdb_id||'\">'|| mrkr_abbrev || '</a>' , " + "          ra.recattrib_source_zdb_id, sup.idsup_supplier_zdb_id , sup.idsup_acc_num,  " + "          src.srcurl_url, src.srcurl_display_text , mrel_zdb_id  " + " 	  from marker_relationship  " + "	       inner join marker_relationship_type " + "                 on mrel_type = mreltype_name " + "               inner join marker " + "                 on mrel_mrkr_2_zdb_id = mrkr_zdb_id  " + "	       inner join marker_types " + "                 on mrkr_type = marker_type " + "	       left outer join record_attribution ra on ra.recattrib_data_zdb_id=mrel_zdb_id " + "	       left outer join int_data_supplier sup on sup.idsup_data_zdb_id=mrel_mrkr_2_zdb_id " + "	       left outer join source_url src on sup.idsup_supplier_zdb_id=src.srcurl_source_zdb_id  " + "          left outer join db_link on mrel_mrkr_2_zdb_id = dblink_linked_recid " + "               and (dblink_fdbcont_zdb_id = 'ZDB-FDBCONT-040412-36' or dblink_fdbcont_zdb_id = 'ZDB-FDBCONT-040412-37')" + "	 where mrel_mrkr_1_zdb_id = :markerZdbId  ";
        ...
        String sql2To1 = " select mrkr_abbrev, mrkr_zdb_id, mrkr_abbrev_order, mrkrtype_type_display,  " + "	       mreltype_2_to_1_comments, " + "          '<a href=\"/'||mrkr_zdb_id||'\">'|| mrkr_abbrev || '</a>' , " + "          ra.recattrib_source_zdb_id, sup.idsup_supplier_zdb_id , sup.idsup_acc_num,  " + "          src.srcurl_url, src.srcurl_display_text  , mrel_zdb_id   " + " 	  from marker_relationship " + "	       inner join marker_relationship_type " + "             	  on mrel_type = mreltype_name " + "               inner join marker " + "                  on mrel_mrkr_1_zdb_id = mrkr_zdb_id  " + "	       inner join marker_types " + "            	  on mrkr_type = marker_type " + "	       left outer join record_attribution ra on ra.recattrib_data_zdb_id=mrel_zdb_id " + "	       left outer join int_data_supplier sup on sup.idsup_data_zdb_id=mrel_mrkr_1_zdb_id " + "	       left outer join source_url src on sup.idsup_supplier_zdb_id=src.srcurl_source_zdb_id  " + "	 where mrel_mrkr_2_zdb_id = :markerZdbId  ";
    ...
    public List<MarkerRelationshipPresentation> getRelatedMarkerOrderDisplayForTypes(Marker marker, boolean is1to2, MarkerRelationship.Type... types) {
        String sql1To2 = " 	select mrkr_abbrev, mrkr_zdb_id, mrkr_abbrev_order, mrkrtype_type_display,  " + "	       mreltype_1_to_2_comments, " + "          '<a href=\"/'||mrkr_zdb_id||'\">'|| mrkr_abbrev || '</a>' , " + "          ra.recattrib_source_zdb_id, sup.idsup_supplier_zdb_id , sup.idsup_acc_num,  " + "          src.srcurl_url, src.srcurl_display_text  , mrel_zdb_id  " + " 	  from marker_relationship  " + "	       inner join marker_relationship_type " + "                 on mrel_type = mreltype_name " + "               inner join marker " + "                 on mrel_mrkr_2_zdb_id = mrkr_zdb_id  " + "	       inner join marker_types " + "                 on mrkr_type = marker_type " + "	       left outer join record_attribution ra on ra.recattrib_data_zdb_id=mrel_zdb_id " + "	       left outer join int_data_supplier sup on sup.idsup_data_zdb_id=mrel_mrkr_2_zdb_id " + "	       left outer join source_url src on sup.idsup_supplier_zdb_id=src.srcurl_source_zdb_id  " + "	 where mrel_mrkr_1_zdb_id = :markerZdbId  ";
        if (types.length > 0) {
    ...
    public List<LinkDisplay> getMarkerLinkDisplay(String dbLinkId) {
        String sql = "SELECT fdbdt.fdbdt_data_type, dbl.dblink_length, dbl.dblink_linked_recid, dbl.dblink_acc_num, fdb.fdb_db_display_name, fdb.fdb_db_query, fdb.fdb_url_suffix, " + "ra.recattrib_source_zdb_id, fdb.fdb_db_significance, dbl.dblink_zdb_id, fdbc.fdbcont_zdb_id, pub.title, fdbdt.fdbdt_display_order, dbl.dblink_acc_num_display " + "FROM db_link dbl  " + "JOIN foreign_db_contains fdbc ON dbl.dblink_fdbcont_zdb_id=fdbc.fdbcont_zdb_id " + "JOIN foreign_db fdb ON fdbc.fdbcont_fdb_db_id=fdb.fdb_db_pk_id " + "JOIN foreign_db_data_type fdbdt ON fdbdt.fdbdt_pk_id = fdbc.fdbcont_fdbdt_id " + "LEFT OUTER JOIN record_attribution ra ON ra.recattrib_data_zdb_id=dbl.dblink_zdb_id " + "JOIN publication pub ON ra.recattrib_source_zdb_id=pub.zdb_id " + "WHERE dbl.dblink_zdb_id = :dbLinkId ";
    ...
    public List<MarkerRelationshipPresentation> getRelatedMarkerDisplayForTypes(Marker marker, boolean is1to2, MarkerRelationship.Type... types) {
        String sql1To2 = " 	select mrkr_abbrev, mrkr_zdb_id, mrkr_abbrev_order, mrkrtype_type_display,  " + "	       mreltype_1_to_2_comments, mrkr_name, " + "          ra.recattrib_source_zdb_id , mrel_zdb_id " + " 	  from marker_relationship  " + "	       inner join marker_relationship_type " + "                 on mrel_type = mreltype_name " + "               inner join marker " + "                 on mrel_mrkr_2_zdb_id = mrkr_zdb_id  " + "	       inner join marker_types " + "                 on mrkr_type = marker_type " + "	       left outer join record_attribution ra on ra.recattrib_data_zdb_id=mrel_zdb_id " + "	 where mrel_mrkr_1_zdb_id = :markerZdbId  ";
        if (types.length > 0) {
            sql1To2 += "	   and mrel_type in (:types) ";
        }
        sql1To2 += "      order by mrel_type, mrkrtype_type_display, mrkr_abbrev_order";

        String sql2To1 = " select mrkr_abbrev, mrkr_zdb_id, mrkr_abbrev_order, mrkrtype_type_display,  " + "	       mreltype_2_to_1_comments, mrkr_name, " + "          ra.recattrib_source_zdb_id , mrel_zdb_id " + " 	  from marker_relationship " + "	       inner join marker_relationship_type " + "             	  on mrel_type = mreltype_name " + "               inner join marker " + "                  on mrel_mrkr_1_zdb_id = mrkr_zdb_id  " + "	       inner join marker_types " + "            	  on mrkr_type = marker_type " + "	       left outer join record_attribution ra on ra.recattrib_data_zdb_id=mrel_zdb_id " + "	 where mrel_mrkr_2_zdb_id = :markerZdbId  ";
        if (types.length > 0) {
            sql2To1 += "	   and mrel_type in (:types) ";
        }
        sql2To1 += "      order by mrel_type, mrkrtype_type_display, mrkr_abbrev_order ";
    ...
    public List<GeneProductsBean> getGeneProducts(String zdbID) {
        String sql = " SELECT dblink_acc_num, REPLACE(extnote_note,'CTRL','<br>') " + " FROM external_note, db_link " + " WHERE extnote_data_zdb_id = dblink_zdb_id " + " AND dblink_linked_recid = :markerZdbID ";
    ...
    public List<InterProProtein> getInterProForMarker(Marker marker) {
        Session session = currentSession();
        String sql = " SELECT  distinct ip " + " FROM InterProProtein ip, MarkerToProtein mtp, ProteinToInterPro pti " + " WHERE ip.ipID = pti.interProID " + " AND pti.uniProtID = mtp.mtpUniProtID " + " AND mtp.marker = :marker order by ip.ipType";
    ...
    public List<String> getIPNames(String uniprot) {
        String sql = " select ip_name " + " from marker_to_protein, protein_to_interpro,interpro_protein " + " where mtp_uniprot_id=:uniprot and mtp_uniprot_id=pti_uniprot_id and pti_interpro_id=ip_interpro_id " + " order by ip_name ";
    ...
    public List<String> getProteinType(Marker gene) {
        String sql = " select distinct ip_name " + " from marker_to_protein, protein_to_interpro,interpro_protein " + " where mtp_mrkr_zdb_id=:markerzdb and mtp_uniprot_id=pti_uniprot_id and pti_interpro_id=ip_interpro_id " + " order by ip_name ";
    ...
    public boolean isFromChimericClone(String zdbID) {
        String sql = "   SELECT count(*) AS count  " + "    FROM marker_relationship " + "    JOIN clone ON clone_mrkr_zdb_id = mrel_mrkr_2_zdb_id " + "    WHERE mrel_mrkr_1_zdb_id = :markerZdbID " + "    AND clone_problem_type = 'Chimeric'";
    ...
    public boolean cloneHasSnp(Clone clone) {
        String sql = "  SELECT DISTINCT snpdattr_pub_zdb_id " + "  FROM snp_download_attribution, snp_download " + "  WHERE snpd_mrkr_zdb_id=:markerZdbID " + "  AND snpd_pk_id=snpdattr_snpd_pk_id " + "  ORDER BY snpdattr_pub_zdb_id";
...
*/
    ...
    public List<String> getMarkerZdbIdsForType(Marker.Type markerType) {
        String hql = " " + " select m.zdbID " + " from Marker m " + " where m.markerType.name = :type " + " ";
    ...
    public List<Marker> getConstructsForGene(Marker gene) {
        ...
        String hql = " select m from MarkerRelationship mr1 , MarkerRelationship  mr2, Marker m " + " where mr1.secondMarker.zdbID=mr2.secondMarker.zdbID " + " and m.zdbID=mr2.firstMarker.zdbID " + " and mr1.firstMarker.zdbID = :markerZdbID " + " and mr1.type = :markerRelationshipType1 " + " and mr2.type in (:markerRelationshipType2) " + "  ";
    ...
    public List<Marker> getGenesforTranscript(Marker tscript) {
        ...
        String hql = " select m from MarkerRelationship mr1,  Marker m " + " where mr1.firstMarker.zdbID=m.zdbID " + " and mr1.secondMarker.zdbID = :markerZdbID " + " and mr1.type = :markerRelationshipType1 ";
    ...
    public List<Marker> getCodingSequence(Marker gene) {
        ...
        String hql = " select m from MarkerRelationship mr1,  Marker m " + " where mr1.secondMarker.zdbID=m.zdbID " + " and mr1.firstMarker.zdbID = :markerZdbID " + " and mr1.type = :markerRelationshipType1 " + " ";
    ...
    public Genotype getStrainForTranscript(String zdbID) {
        ...
        String hql = " select g from Genotype g, ProbeLibrary pl , MarkerRelationship mr, Clone c   " + "where pl=c.probeLibrary " + "and g=pl.strain " + "and mr.firstMarker=c " + "and mr.secondMarker.zdbID = :zdbID " + "and mr.type = :mrType  ";

        return HibernateUtil.currentSession().createQuery(hql, Genotype.class).setParameter("zdbID", zdbID).setParameter("mrType", MarkerRelationship.Type.CLONE_CONTAINS_TRANSCRIPT).uniqueResult();
    }
    ...
    public List<LinkDisplay> getVegaGeneDBLinksTranscript(Marker gene, DisplayGroup.GroupName summaryPage) {
        ...
        String sql = "SELECT DISTINCT " + "        fdbdt.fdbdt_data_type, " + "        dbl.dblink_length, " + "        dbl.dblink_linked_recid," + "        dbl.dblink_acc_num," + "        fdb.fdb_db_name," + "        fdb.fdb_db_query," + "        fdb.fdb_url_suffix," + "        ra.recattrib_source_zdb_id," + "        fdb.fdb_db_significance," + "        dbl.dblink_zdb_id, " + "        fdbc.fdbcont_zdb_id " + "    FROM" + "        db_link dbl  " + "    JOIN" + "        foreign_db_contains fdbc " + "            ON dbl.dblink_fdbcont_zdb_id=fdbc.fdbcont_zdb_id " + "    JOIN" + "        foreign_db fdb " + "            ON fdbc.fdbcont_fdb_db_id=fdb.fdb_db_pk_id " + "    JOIN" + "        foreign_db_data_type fdbdt " + "            ON fdbc.fdbcont_fdbdt_id=fdbdt.fdbdt_pk_id " + "    JOIN " + "    marker_relationship mr " + "    ON mr.mrel_mrkr_2_zdb_id=dbl.dblink_linked_recid" + "    LEFT OUTER JOIN" + "        record_attribution ra " + "            ON ra.recattrib_data_zdb_id=dbl.dblink_zdb_id " + "    WHERE" + "       mr.mrel_mrkr_1_zdb_id = :markerZdbId " + "        AND " + "        mr.mrel_type='gene produces transcript'" + "        AND " + "        fdb.fdb_db_name='VEGA'" + // Ensembl
                     " ";
    ...
    public List<MarkerRelationshipPresentation> getClonesForGeneTranscripts(String zdbID) {
        ...
        String sql = " SELECT m.mrkr_abbrev, m.mrkr_zdb_id, m.mrkr_abbrev_order, mt.mrkrtype_type_display, " + "mrt.mreltype_2_to_1_comments, " + "'<a href=\"/'||mrkr_zdb_id||'\">'|| mrkr_abbrev || '</a>' , " + "ra.recattrib_source_zdb_id, sup.idsup_supplier_zdb_id , sup.idsup_acc_num,  " + "src.srcurl_url, src.srcurl_display_text , mrct.mrel_zdb_id   " + "FROM marker_relationship mrgt " + "JOIN marker_relationship mrct ON mrct.mrel_mrkr_2_zdb_id=mrgt.mrel_mrkr_2_zdb_id " + "JOIN marker_relationship_type mrt ON mrt.mreltype_name=mrct.mrel_type " + "JOIN marker m ON mrct.mrel_mrkr_1_zdb_id=m.mrkr_zdb_id " + "JOIN marker_types mt ON m.mrkr_type = mt.marker_type " + "LEFT OUTER JOIN record_attribution ra ON ra.recattrib_data_zdb_id=mrct.mrel_zdb_id " + "LEFT OUTER JOIN int_data_supplier sup ON sup.idsup_data_zdb_id=mrct.mrel_mrkr_1_zdb_id " + "LEFT OUTER JOIN source_url src ON sup.idsup_supplier_zdb_id=src.srcurl_source_zdb_id  " + "WHERE mrgt.mrel_mrkr_1_zdb_id = :markerZdbId " + "AND mrct.mrel_type='clone contains transcript' " + "AND mrgt.mrel_type='gene produces transcript' " + "ORDER BY m.mrkr_type ASC , m.mrkr_abbrev_order ASC ";
...
//                "and mrgt.mrel_type='gene produces transcript' "  +
                     "AND mrct.mrel_type=:markerRelationshipType1 " + "AND mrgt.mrel_type=:markerRelationshipType2  " + "ORDER BY m.mrkr_type ASC , m.mrkr_abbrev_order ASC ";
    ...
    public List<Genotype> getMutantsAndTgsByGene(String geneID) {
        ...
        String hql = "select distinct mutant from Genotype mutant, GenotypeFeature genoFtr, FeatureMarkerRelationship fmRel " + "      where fmRel.marker.zdbID = :geneID " + "        and fmRel.feature = genoFtr.feature" + "        and genoFtr.genotype = mutant" + "        and mutant.wildtype = 'f'" + "   order by mutant.nameOrder ";
    ...
    public List<Feature> getFeaturesBySTR(Marker sequenceTargetingReagent) {
        ...
        String hql = "select distinct feat from Feature feat, FeatureMarkerRelationship fmRel " + "      where fmRel.marker = :str " + "        and fmRel.feature = feat" + "        and fmRel.type = 'created by'" + "   order by feat.name ";
    ...
    public List<LookupEntry> getConstructComponentsForString(String lookupString, String zdbId) {
        ...
        String sqlQuery = "SELECT mrkr_abbrev AS abbrev, mrkr_type AS type FROM marker, record_attribution ra,marker_type_group_member m " + "WHERE " + "lower(mrkr_abbrev) LIKE :lookupString " + "AND mrkr_type=m.mtgrpmem_mrkr_type AND m.mtgrpmem_mrkr_type_group IN ('CONSTRUCT_COMPONENTS') " + "AND mrkr_zdb_id=ra.recattrib_data_zdb_id AND ra.recattrib_source_type = :standard AND ra.recattrib_source_zdb_id = :pubZdbID " + "UNION " + "SELECT cv_term_name AS abbrev,cv_name_definition AS type FROM controlled_vocabulary " + "WHERE lower(cv_term_name) LIKE :lookupString ";
    ...
    public List<Marker> getRelatedMarkersForTypes(Marker marker, MarkerRelationship.Type... types) {
        Query<Marker> query = HibernateUtil.currentSession().createQuery("select m from  Marker as m, MarkerRelationship as rel " + "where rel.firstMarker = :marker  and rel.secondMarker = m and " + "rel.type in :relationshipTypes " + "order by rel.markerRelationshipType, m.markerType, m.abbreviationOrder", Marker.class);
        query.setParameter("marker", marker);
        query.setParameterList("relationshipTypes", types);
        List<Marker> list = query.list();
        query = HibernateUtil.currentSession().createQuery("select m from  Marker as m, MarkerRelationship as rel " + "where rel.secondMarker = :marker  and rel.firstMarker = m and " + "rel.type in :relationshipTypes " + "order by rel.markerRelationshipType, m.markerType, m.abbreviationOrder", Marker.class);
        query.setParameter("marker", marker);
        query.setParameterList("relationshipTypes", types);
    ...
    public List<Marker> getRelatedGenesViaTranscript(Marker marker, MarkerRelationship.Type relType1, MarkerRelationship.Type relType2) {
        Query<Marker> query = HibernateUtil.currentSession().createQuery("select distinct m from  Marker as m, MarkerRelationship as rel1,MarkerRelationship as rel2 " + "where rel1.firstMarker = :marker and " + "rel1.type =:relType1 and " + "rel1.secondMarker=rel2.secondMarker " + "and rel2.type=:relType2 and " + " rel2.firstMarker=m", Marker.class);
        query.setParameter("marker", marker);
    ...
    public Marker getMarkerByFeature(Feature feature) {
        String hql = "select fmr.marker from FeatureMarkerRelationship as fmr " + "where " + " fmr.feature = :feature " + " and fmr.type in (:types)";
        Query<Marker> query = HibernateUtil.currentSession().createQuery(hql, Marker.class);
    ...
    public String getAccessionNumber(Marker marker, Database.AvailableAbbrev database) {
        String hql = "from MarkerDBLink " + "where referenceDatabase.primaryBlastDatabase.abbrev = :database " + "and marker = :marker ";
    ...
    public List<TranscriptPresentation> getTranscriptsForGeneId(String geneZdbId) {
        ...
        String hql = "select markerRelationship.secondMarker.zdbID, markerRelationship.secondMarker.name from MarkerRelationship markerRelationship " + "      where markerRelationship.type = :type  " + "        and markerRelationship.firstMarker.zdbID = :geneID " + "   order by markerRelationship.secondMarker.abbreviationOrder ";
    ...
    public List<SequenceTargetingReagentLookupEntry> getSequenceTargetingReagentForString(String lookupString, String type) {
        String hql = " select mo from Marker mo " + "where " + "lower(mo.abbreviation) like :lookupString " + "and mo.markerType.name = :type " + "order by mo.abbreviation  ";
        return HibernateUtil.currentSession().createQuery(hql).setParameter("lookupString", "%" + lookupString.toLowerCase() + "%").setParameter("type", type).setResultTransformer(new BasicTransformerAdapter() {
            @Override
    ...
    public List<TargetGenePresentation> getTargetGenesForSequenceTargetingReagent(SequenceTargetingReagent sequenceTargetingReagent) {
        ...
        String hql = "select markerRelationship.secondMarker.zdbID, markerRelationship.secondMarker.abbreviation from MarkerRelationship markerRelationship " + "      where markerRelationship.type = :type  " + "        and markerRelationship.firstMarker.zdbID = :sequenceTargetingReagentID " + "   order by markerRelationship.secondMarker.abbreviationOrder ";
    ...
    public List<Marker> getSecondMarkersByFirstMarkerAndMarkerRelationshipType(Marker firstMarker, MarkerRelationship.Type relationshipType) {
        ...
        String hql = "select rel.secondMarker from MarkerRelationship as rel  " + "where rel.firstMarker = :firstMarker " + "and rel.type = :type " + "order by rel.secondMarker.abbreviationOrder";
    ...
    public List<MarkerRelationship> getMarkerRelationshipBySecondMarker(Marker secondMarker) {
        ...
        String hql = "select rel from MarkerRelationship as rel  " + "where rel.secondMarker = :secondMarker " + "order by rel.firstMarker.abbreviationOrder";
    ...
    public PaginationResult<Marker> getRelatedMarker(Marker marker, Set<MarkerRelationship.Type> types, PaginationBean paginationBean) {
        ...
        String hql = "select rel.secondMarker from MarkerRelationship as rel  " + "where rel.firstMarker = :firstMarker " + "and rel.type in (:typeList) " + "order by rel.secondMarker.abbreviationOrder";
        ...
        // second related elements
        hql = "select rel.firstMarker from MarkerRelationship as rel  " + "where rel.secondMarker = :secondMarker " + "and rel.type in (:typeList) " + "order by rel.firstMarker.abbreviationOrder";
    ...
    public List<OmimPhenotype> getOmimPhenotype(Marker marker) {
        Session session = HibernateUtil.currentSession();
        String sql = "FROM OmimPhenotype " + "WHERE ortholog.zebrafishGene = :gene " + "AND ortholog.ncbiOtherSpeciesGene.organism.commonName = :organism ";
    ...
    public int getCrisprCount(String geneAbbrev) {
        Session session = currentSession();
        String hql = "select rel.firstMarker from MarkerRelationship as rel  " + "where rel.secondMarker.zdbID = :geneAbbrev and rel.type = :type " + "and rel.firstMarker.markerType like '%CRISPR%'  order by rel.secondMarker.abbreviationOrder";
        Query<Marker> query = session.createQuery(hql, Marker.class);
    ...
    public void copyStrSequence(SequenceTargetingReagent str1, SequenceTargetingReagent str2) {
        ...
        HibernateUtil.currentSession().createSQLQuery("UPDATE marker_sequence " + "SET seq_sequence = :seq_sequence " + "WHERE seq_mrkr_zdb_id = :zdbID ").setParameter("seq_sequence", seq1).setParameter("zdbID", str2.getZdbID()).executeUpdate();
        String seq2;
        if (str1.getSequence().getSecondSequence() != null) {
            seq2 = str1.getSequence().getSecondSequence();
            HibernateUtil.currentSession().createSQLQuery("UPDATE marker_sequence " + "SET seq_sequence_2 = :seq_sequence_2 " + "WHERE seq_mrkr_zdb_id = :zdbID ").setParameter("seq_sequence_2", seq2).setParameter("zdbID", str2.getZdbID()).executeUpdate();
        }
    }

    @Override
    public List<LookupEntry> getRegionListForString(String lookupString, String type) {
        String hql = " select region from Marker region " + "where " + "lower(region.abbreviation) like :lookupString " + "and region.markerType.name = :type " + "order by region.abbreviation  ";
        return HibernateUtil.currentSession().createQuery(hql).setParameter("lookupString", "%" + lookupString.toLowerCase() + "%").setParameter("type", type).setResultTransformer(new BasicTransformerAdapter() {
            @Override
    ...
    public List<AntibodyLookupEntry> getAntibodyForString(String lookupString, String type) {
        String hql = " select ab from Marker ab " + "where " + "lower(ab.abbreviation) like :lookupString " + "and ab.markerType.name = :type " + "order by ab.abbreviation  ";
        return HibernateUtil.currentSession().createQuery(hql).setParameter("lookupString", "%" + lookupString.toLowerCase() + "%").setParameter("type", type).setResultTransformer(new BasicTransformerAdapter() {
            @Override
            public Object transformTuple(Object[] tuple, String[] sequenceTargetingReagents) {
