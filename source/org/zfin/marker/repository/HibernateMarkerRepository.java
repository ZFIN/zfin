...
public class HibernateMarkerRepository implements MarkerRepository {
    ...
    public List<Transcript> getTranscriptsForNonCodingGenes() {
        ...
        return currentSession().createQuery(hql, Transcript.class)
            .setParameter("markerRelationshipType1", GENE_PRODUCES_TRANSCRIPT)
...
//                "and mrgt.mrel_type='gene produces transcript' "  +
    ...
    public Map<Marker, List<Transcript>> getAllTranscripts() {
        String hql = """
            select mrel, transcript from MarkerRelationship as mrel, Transcript transcript
            where mrel.type = :type
            and mrel.secondMarker = transcript
            """;
        ...
        Map<Marker, List<Transcript>> map = new HashMap<>();
        response.forEach(tuple -> {
            List<Transcript> list = map.computeIfAbsent(tuple.get(0, MarkerRelationship.class).getFirstMarker(), marker -> new ArrayList<>());
