...
//    private Image removeSequenceButton = new Image(imageURL+"minimize.gif") ;
    ...
    protected void initGUI(){
        ...
        removeSequenceButton.addClickListener(new ClickListener(){
...
//    }
    ...
    public String checkSequence(){
        ...
        if(type.equals(PROTEIN_SEQUENCE)){
            char[] chars = sequenceHTML.getText().toUpperCase().toCharArray() ;
            for(int i = 0 ; i < chars.length ; i++){
                if(Character.isLetter(chars[i])){
                    switch (chars[i]){
                        case 'G': case 'P': case 'A': case 'V': case 'L': case 'I':
                        case 'M': case 'C': case 'F': case 'Y': case 'W': case 'H':
                        case 'K': case 'R': case 'Q': case 'N': case 'E': case 'D':
