...
--changeset pm:DLOAD-621b





update featuregenedata
...
delete from featuregenedata where allele in (select feature_abbrev from feature);

insert into record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id) select distinct geneid, 'ZDB-PUB-130425-4' from featuregenedata where geneid not in (select recattrib_data_zdb_id from record_attribution where recattrib_source_zdb_id='ZDB-PUB-130425-4');

insert into record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id) select distinct feature_zdb_id, 'ZDB-PUB-130425-4' from featuregenedata, feature where feature_abbrev=allele and feature_zdb_id not in (select recattrib_data_zdb_id from record_attribution where recattrib_source_zdb_id='ZDB-PUB-130425-4');

insert into record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id) select fmrel_zdb_id , 'ZDB-PUB-130425-4' from feature, feature_marker_relationship where feature_zdb_id=fmrel_ftr_zdb_id and feature_abbrev in (select allele from featuregenedata) and fmrel_zdb_id not in (select recattrib_data_zdb_id from record_attribution where recattrib_source_zdb_id='ZDB-PUB-130425-4');
...
create table pre_feature (
        ...
        preftr_gene_ensdargid varchar(50),
...
insert into pre_feature (
  ...
  select distinct allele,
                  ...
                  SUBSTR(allele,3),
                  fp_pk_id
    from featuregenedata, feature_prefix
     where fp_prefix = 'sa'
     and allele like 'sa%'
...
insert into pre_db_link (
        ...
        predblink_acc_num_display,
        predblink_fdbcont_zdb_id)
  select distinct preftr_gene_zdb_id, preftr_gene_ensdargid,preftr_gene_ensdargid, 'ZDB-FDBCONT-061018-1'
    from pre_feature;
