...
insert into vocabulary (v_name, v_description) VALUES ('transcript annotation method','All allowed values for transcript annotation method');

insert into vocabulary_term (vt_name, vt_v_id) VALUES ('Havana', (select v_id from vocabulary));
insert into vocabulary_term (vt_name, vt_v_id) VALUES ('Ensembl', (select v_id from vocabulary));

alter table transcript add column tscript_vocab_id integer;

ALTER TABLE vocabulary_term ADD CONSTRAINT vocabulary_term_pk PRIMARY KEY(vt_id);
...
update transcript set tscript_vocab_id = 1
...
from genotype as g, probe_library, clone, marker_relationship, transcript
where probelib_strain_zdb_id = g.geno_zdb_id AND clone_probelib_zdb_id = probelib_zdb_id AND mrel_mrkr_1_zdb_id = clone_mrkr_zdb_id AND mrel_mrkr_2_zdb_id = tscript_mrkr_zdb_id and mrel_type = 'clone contains transcript';
...
from transcript_strain_raw group by tscript_mrkr_zdb_id,geno_zdb_Id, geno_display_name order by number_of_clones desc;

update transcript as t set tscript_genotype_zdb_id = subquery.geno_zdb_id
