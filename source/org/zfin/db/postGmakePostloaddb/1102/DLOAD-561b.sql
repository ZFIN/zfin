...
delete from featurenogenedata where allele in (select feature_abbrev from feature);


insert into record_attribution (recattrib_data_zdb_id, recattrib_source_zdb_id) select distinct feature_zdb_id, 'ZDB-PUB-130425-4' from featurenogenedata, feature where feature_abbrev=allele and feature_zdb_id not in (select recattrib_data_zdb_id from record_attribution where recattrib_source_zdb_id='ZDB-PUB-130425-4');
...
insert into pre_feature (
  ...
  select distinct allele,
                  ...
                  SUBSTR(allele,3),
                  fp_pk_id
    from featurenogenedata, feature_prefix
     where fp_prefix = 'sa'
     and allele like 'sa%'
