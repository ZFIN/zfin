...
create table fishMisfortune (fm_fish_zdb_id varchar(50),
       	     		        fm_fish_display_name varchar(255),
       	     		    	fm_misfortune_count int default 0,
				fm_genox_zdb_id varchar(50),
				fm_affected_gene_count int default 0,
				fm_str_count int default 0 ,
				fm_experiment_condition_count int default 0 ,
				fm_pub_zdb_id varchar(50),
				fm_feature_count int default 0,
				fm_misfortune_count_minus_one int default 0)
fragment by round robin in tbldbs1, tbldbs2, tbldbs3
extent size 2048 next size 2048;


create index fm_misfortune_count_index
 on fishMisfortune (fm_misfortune_count)
 using btree in idxdbs1;

create index fm_misfortune_count_minus_one_index
 on fishMisfortune (fm_misfortune_count_minus_one)
 using btree in idxdbs1;

create index fm_pub_zdb_id_index
 on fishMisfortune (fm_pub_zdb_id)
 using btree in idxdbs3;

create index fm_fish_zdb_id_index
 on fishMisfortune (fm_fish_zdb_id)
 using btree in idxdbs3;

create unique index fm_fish_pub_zdb_id_index
 on fishMisfortune (fm_fish_zdb_id, fm_genox_zdb_id, fm_pub_zdb_id)
