...
create temp table tmp_new_img_name_a (img_zdb_id text,
       pub_zdb_id text,
       year_string text)
;

insert into tmp_new_img_name_a (img_zdb_id, pub_zdb_id, year_string)
 select img_zdb_id, 
        fig_source_zdb_id, 
...
create index img_zdb_id_index_a on tmp_new_img_name_a (img_zdb_id);

create index pub_zdb_id_index_a on tmp_new_img_name_a (pub_zdb_id);

update image 
  set img_image_with_annotation = (select year_string||'/'||pub_zdb_id||'/'||img_image_with_annotation
                    from tmp_new_img_name_a where tmp_neW_img_name_a.img_zdb_id = image.img_zdb_id)
