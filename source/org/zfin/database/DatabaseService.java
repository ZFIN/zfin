...
public class DatabaseService {
            ...
            if (statement.isLoadStatement()) {
                if (statement.isSelectIntoStatement()) {
                    ...
                    if (data == null) {
                        ...
                        if (statement.getLoadFileName() == null)
                            continue;
                        File loadFile = new File(statement.getLoadFileName());
                        if (!loadFile.exists())
                            continue;
                        FileReader fr = null;
    ...
    private void writeToFile(List<List<String>> dataReturn, File unloadFile, String delimiter) {
        ...
        try {
            fileWriter = new FileWriter(unloadFile);
            if (unloadFile.getName().toLowerCase().endsWith("gff3")) {
                fileWriter.write("##gff-version 3");
                fileWriter.write(System.lineSeparator());
            }
            for (List<String> lineElements : dataReturn) {
                String line = String.join(delimiter, lineElements);
