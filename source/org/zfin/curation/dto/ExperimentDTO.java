...
public class ExperimentDTO implements IsSerializable, Comparable<ExperimentDTO> {
    ...
    public int hashCode() {
        int code = 43;
        if (geneZdbID != null)
            code += geneZdbID.hashCode();
        if (fishID != null)
            code += fishID.hashCode();
        if (environment != null)
            code += environment.hashCode();
        if (assay != null)
            code += assay.hashCode();
        if (antibodyID != null)
            code += antibodyID.hashCode();
        if (genbankID != null)
            code += genbankID.hashCode();
        return code;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!StringUtils.equalsWithNullString(geneZdbID, experiment.geneZdbID))
            return false;
        if (!StringUtils.equals(fishName, experiment.fishName))
            return false;
        if (!StringUtils.equals(environment, experiment.environment))
            return false;
        if (!StringUtils.equals(assay, experiment.assay))
            return false;
        if (!StringUtils.equalsWithNullString(antibodyID, experiment.antibodyID))
            return false;
