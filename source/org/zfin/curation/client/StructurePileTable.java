...
import org.zfin.framework.presentation.client.ListItem;
import org.zfin.framework.presentation.client.PostComposedPart;
import org.zfin.framework.presentation.client.UnorderedList;
...
import org.zfin.framework.presentation.dto.RelatedPileStructureDTO;
...
class StructurePileTable extends ZfinFlexTable {
    ...
    public void onClick(ClickEvent event) {
        ...
        if (expressionModule.getSelectedExpressions() == null || expressionModule.getSelectedExpressions().size() == 0)
            return;
    ...
    protected void checkNeedForAlternativeStructures(PileStructureDTO selectedPileStructure, int row) {
        ...
        if (getSelectedExpressions().size() > 0 && add.getValue()) {
    ...
    public void setHeaderRow() {
        ...
        for (HeaderName name : headerNames) {
            if (name.getIndex() == 0) {
                ...
                getCellFormatter().setStyleName(rowIndex, name.index, WidgetUtil.RED_MOLDIFIER);
            } else
                setText(rowIndex, name.index, name.getName());
        }
    }
    ...
    public void highlightStructure(int row, boolean highlight) {
        ...
        // make red if sturcture is not expressed
