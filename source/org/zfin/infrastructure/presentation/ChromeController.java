 ...
 *
 * Caching: the fragments are cached on the client by zfin-chrome.js (localStorage,
 * 5-minute TTL keyed on login state) -- see loadFragment() there. That fetch uses
 * {@code cache: "no-store"}, so no HTTP cache headers are set here; HTTP caching of
 * this login-aware, Tomcat-proxied response proved unreliable in the browser.
 *
