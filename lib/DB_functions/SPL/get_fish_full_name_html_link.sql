create function get_fish_full_name_html_link( fishZdbId varchar(50) )
 
  returning lvarchar;

  -- --------------------------------------------------------------------- 
  -- Given the ZDB ID of a fish, this returns the full name of the fish
  ...
  -- 
  -- RETURNS: 
  --   Full name of fish with proper HTML formatting, embedded in an HTML link
  ...
  return
    '<a href="/<!--|WEBDRIVER_PATH_FROM_ROOT|-->?MIval=aa-fishview.apg&OID=' ||
      fishZdbId || '">' ||
      get_fish_full_name_html(fishZdbId) ||
    '</a>';
