create function get_mrkr_name_html( mrkrZdbId varchar(50) )
  ...
  select mrkr_name, mrkr_type
    into mrkrName, mrkrType
    from marker
  ...
  if mrkrName is null then
    ...
    if exists 
      ...
      let mrkrNameHtml = '<span class="genedom">' || mrkrName || '</span>';
    else 
      -- we have something else, display it as such
      let mrkrNameHtml = '<span class="nongenedommarker">' || mrkrName || '</span>';
    end if
