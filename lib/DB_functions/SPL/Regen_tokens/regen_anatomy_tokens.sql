create dba function "informix".regen_anatomy_tokens() returning integer
  ...
  --       /tmp/regen_anatomy_tokens_exception_<!--|DB_NAME|--> for details.
  --
  -- EFFECTS:
  --   Success:
  ...
  --         /tmp/regen_anatomy_tokens_exception_<!--|DB_NAME|--> 
  ...
  --
  -- 2. Display additional messages to the /tmp/regen_anatomy_tokens_exception
  ...
  begin
	...
	-- Change the mode of the regen_anatomy_tokens_exception file.  This is
	...
	system '/bin/chmod 666 /tmp/regen_anatomy_tokens_exception_<!--|DB_NAME|-->';
	...
	-- figure out what went wrong.
        ...
        let zdbFlagReturn = release_zdb_flag("regen_anatomy_tokens");
	return -1;
      end
    end exception;
    ...
    if grab_zdb_flag("regen_anatomy_tokens") <> 0 then
      return 1;
    end if
    ...
    create table all_anatomy_tokens_new
      (
        ...
        anattok_anatitem_zdb_id  varchar(50) not null
      )
    ...
    -- -------------------------------------------------------------------
    --   Populate tokenize input table with all possible anatomy names
    -- ------------------------------------------------------------------
  ...
  if release_zdb_flag("regen_anatomy_tokens") <> 0 then
    return 1;
  end if
...
grant execute on function "informix".regen_anatomy_tokens () 
