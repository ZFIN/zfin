...
CREATE OR REPLACE VIEW genotype_name_components_from_relationships_part_1 AS
...
CREATE OR REPLACE VIEW genotype_name_components_from_relationships_part_2 AS
        SELECT DISTINCT
            get_feature_and_marker_abbrev_display(feature_zdb_id, mrkr_zdb_id) AS fad,
            zyg_allele_display,
            ...
            feature_type AS feature_type,
            zyg_abbrev AS zyg_abbrev,
            feature_abbrev AS feature_abbrev2,
        ...
        WHERE genofeat_feature_zdb_id = feature_zdb_id
          ...
          AND NOT EXISTS (
              ...
              AND fmrel_type = 'is allele of')
      AND (
                feature_Type = 'DEFICIENCY'
            OR
                (feature_Type != 'DEFICIENCY'
                    AND fmrel_type NOT IN ('is allele of', 'created by', 'markers present', 'markers moved'))
            OR
                feature_type in ('TRANSLOC', 'INVERSION')
        );

CREATE OR REPLACE VIEW genotype_name_components_from_relationships AS
    SELECT * FROM genotype_name_components_from_relationships_part_1
        UNION
    SELECT * FROM genotype_name_components_from_relationships_part_2;
